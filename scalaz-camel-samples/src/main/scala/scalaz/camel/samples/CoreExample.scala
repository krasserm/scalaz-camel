...
object CoreExample {
  ...
  def run: Unit = {
    ...
    import CoreExampleAsserts.assertOrderProcessed
    ...
    {
      assertOrderProcessed { Message(template.requestBody("direct:place-order", order)).success }
...
object CoreExampleAsserts {
  ...
  def assertOrderProcessed(validation: => Camel.MessageValidation)(implicit cm: ContextMgnt) {
    ...
    validation match {
      case Success(m) => assert(m.body == "order accepted")
